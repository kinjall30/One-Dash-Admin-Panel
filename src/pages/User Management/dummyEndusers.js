...
class Enduser extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            dynamic_description: "",
            pageToken: ['null']
            // next: <div onClick={() => this.nextPaginations()}>Next</div>
        }
    ...
    componentDidMount(){
        ...
        myHeaders.append("Authorization", "Bearer "+localStorage.getItem("token"));
          ...
          fetch("http://44.196.105.0:3000/endusers?paginationToken=" + this.state.pageToken[0], {
              method: 'GET',
              headers: myHeaders,
          })
        ...
        for(let i=0; i< data.body.length; i++){
            ...
            this.setState({users: array, nextPagination: data.paginationToken, pageToken: this.state.pageToken.push(data.paginationToken)})
            console.log("pagination",this.state.nextPagination)    
        })

    //   this.setState({
    //     next: this.nextPaginations()
    //   })
    }
    ...
    getUsers(pageNo){
        ...
        //     url = 'http://44.196.105.0:3000/endusers?paginationToken=null'
        // }else{
        ...
        fetch(url, {
            ...
            .then(data => {
                ...
                var newArray = array2.concat(array)
                this.setState({users:newArray })
                this.setState({ nextPagination: data.paginationToken})
                console.log(data)
    ...
    previousPagination(){
        // this.setState({
        //     nextPagination: this.state.pagination
        // })
    ...
    render() {
        ...
        var pages = []
        for(let i=0; i<this.state.pageToken; i++){
            // pages += <PaginationItem active><PaginationLink href="#">{i+1}</PaginationLink></PaginationItem>
            pages.push( <PaginationItem><PaginationLink onClick ={() => this.pageClick(i)}> {i + 1}</PaginationLink></PaginationItem>)
        }
                            ...
                            <Card>
                                <CardBody>
                                    <MDBDataTable
                                        ...
                                        striped
                                        data={data}
                                        paginationLabel={['Prev', this.state.next]}
                                        // nextPage = {() => this.nextPaginations()}
                                        // paging={false}
                                    />
                                    <Col lg="12">
                                        <Card>
